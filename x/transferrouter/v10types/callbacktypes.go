...
const (
	ModuleName = "copied_ibccallbacks"
...
func GetSourceCallbackData(
	...
	packetData, err := packetDataUnmarshaler.UnmarshalPacketData(packet.GetData())
	if err != nil {
		return CallbackData{}, false, nil
	}

	return GetCallbackData(packetData, "ics20-1", packet.GetSourcePort(), ctx.GasMeter().GasRemaining(), maxGas, SourceCallbackKey)
}
...
func GetDestCallbackData(
	...
	return GetCallbackData(packetData, "ics20-1", packet.GetSourcePort(), ctx.GasMeter().GasRemaining(), maxGas, DestinationCallbackKey)
}
